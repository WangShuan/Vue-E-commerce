<template>
  <div class="box">
    <div class="mx-2" v-if="$route.path === '/checkout'">
      ...
      <template v-if="cart.total !== 0">
        <h5 class="mt-5 text-success">請填寫收件資料</h5>
        <div class="row justify-content-center">
          <form class="col-md-8 my-4 text-dark text-left" @submit.prevent="addOrder">
            ...
            <div class="form-group">
              <label for="useraddress">寄件地址</label>
              <input
                type="text"
                class="form-control"
                name="address"
            ...
            <div class="form-group">
              <label for="comment">給賣家的話(商品備註)</label>
              <textarea
                name="message"
                id="comment"
                class="form-control"
                cols="30"
                rows="5"
                placeholder="請輸入至少兩個字內容"
                v-model="form.message"
                v-validate="'required|min:2'"
                :class="{ 'is-invalid': errors.has('message') }"
              ></textarea>
              <span class="text-danger" v-if="errors.has('message')">
                {{
            ...
            <div class="text-right">
              <button class="btn btn-danger">資料送出</button>
            </div>
          </form>
        </div>
      </template>
